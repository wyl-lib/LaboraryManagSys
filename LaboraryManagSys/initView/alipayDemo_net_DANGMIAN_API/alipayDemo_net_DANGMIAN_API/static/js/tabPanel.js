...
+function() {
	var Tab = function(el,options){
        ...
        this.el = el;
		this.init();
} /// <reference path="../../pagepay/pagepay.aspx" />
	...
	Tab.prototype.init = function(){
		...
		var li = '<li role="presentation"><a href="" aria-controls="home" role="tab" data-toggle="tab">统一收单接口</a></li>';
		var tab = '<div class="tab-pane" role="tabpanel"></div>'
		var liAry = [],tabAry = [];
		for(var i in data){
			var $li = $(li),$tab = $(tab);
			var component = this.getFormPanel(data[i].apiInParam,'demo/'+data[i].apiNameFirstLower+'Service.ashx');

			$li.find('a').html(data[i].apiZhName).attr('href','#'+data[i].apiNameFirstLower).end();
